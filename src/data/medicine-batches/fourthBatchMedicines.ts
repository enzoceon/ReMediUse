import { Medicine } from "@/components/medicine/MedicineCard";
import { getReliableImage } from "../images";

// Fourth batch of medicines (47-96)
export const fourthBatchMedicines: Medicine[] = [
  {
    id: "47",
    name: "Amlodipine 10mg",
    image: getReliableImage(),
    price: 195,
    description: "Calcium channel blocker used to treat high blood pressure and chest pain.",
    expiryDate: "2025-07-22",
    location: "Shimla, Himachal Pradesh",
    category: "Blood Pressure",
    isDonation: false
  },
  {
    id: "48",
    name: "Clonazepam 0.5mg",
    image: getReliableImage(),
    price: 215,
    description: "Benzodiazepine medication used to treat seizures and panic disorders.",
    expiryDate: "2025-06-18",
    location: "Gangtok, Sikkim",
    category: "Anti-Anxiety",
    isDonation: false
  },
  {
    id: "49",
    name: "Methotrexate 2.5mg",
    image: getReliableImage(),
    price: 310,
    description: "Immunosuppressant drug used to treat autoimmune conditions and certain cancers.",
    expiryDate: "2025-05-29",
    location: "Imphal, Manipur",
    category: "Immunosuppressant",
    isDonation: false
  },
  {
    id: "50",
    name: "Warfarin 5mg",
    image: getReliableImage(),
    price: 180,
    description: "Anticoagulant used to prevent blood clots.",
    expiryDate: "2025-08-14",
    location: "Itanagar, Arunachal Pradesh",
    category: "Anticoagulant",
    isDonation: false
  },
  {
    id: "51",
    name: "Fluoxetine 20mg",
    image: getReliableImage(),
    price: 255,
    description: "Selective serotonin reuptake inhibitor (SSRI) antidepressant.",
    expiryDate: "2025-07-11",
    location: "Aizawl, Mizoram",
    category: "Antidepressant",
    isDonation: false
  },
  {
    id: "52",
    name: "Venlafaxine 75mg",
    image: getReliableImage(),
    price: 290,
    description: "Serotonin-norepinephrine reuptake inhibitor (SNRI) antidepressant.",
    expiryDate: "2025-09-17",
    location: "Kohima, Nagaland",
    category: "Antidepressant",
    isDonation: false
  },
  {
    id: "53",
    name: "Probiotics Capsules",
    image: getReliableImage(),
    price: null,
    description: "Dietary supplement to support digestive health.",
    expiryDate: "2025-10-05",
    location: "Agartala, Tripura",
    category: "Digestive Health",
    isDonation: true
  },
  {
    id: "54",
    name: "Cephalexin 500mg",
    image: getReliableImage(),
    price: 245,
    description: "Cephalosporin antibiotic used to treat bacterial infections.",
    expiryDate: "2025-08-22",
    location: "Pondicherry",
    category: "Antibiotic",
    isDonation: false
  },
  {
    id: "55",
    name: "Albuterol Inhaler",
    image: getReliableImage(),
    price: 420,
    description: "Bronchodilator used to relieve asthma symptoms.",
    expiryDate: "2025-07-19",
    location: "Daman",
    category: "Asthma",
    isDonation: false
  },
  {
    id: "56",
    name: "Montelukast 5mg",
    image: getReliableImage(),
    price: 235,
    description: "Leukotriene receptor antagonist used to prevent asthma symptoms.",
    expiryDate: "2025-09-11",
    location: "Silvassa, Dadra & Nagar Haveli",
    category: "Asthma",
    isDonation: false
  },
  {
    id: "57",
    name: "Fexofenadine 180mg",
    image: getReliableImage(),
    price: 175,
    description: "Antihistamine used to relieve allergy symptoms.",
    expiryDate: "2025-08-07",
    location: "Kavaratti, Lakshadweep",
    category: "Antihistamine",
    isDonation: false
  },
  {
    id: "58",
    name: "Sitagliptin 100mg",
    image: getReliableImage(),
    price: 340,
    description: "DPP-4 inhibitor used to treat type 2 diabetes.",
    expiryDate: "2025-06-14",
    location: "Port Blair, Andaman & Nicobar",
    category: "Diabetes",
    isDonation: false
  },
  {
    id: "59",
    name: "Omega 3-6-9 Capsules",
    image: getReliableImage(),
    price: null,
    description: "Dietary supplement to support heart and brain health.",
    expiryDate: "2025-10-18",
    location: "Diu",
    category: "Supplement",
    isDonation: true
  },
  {
    id: "60",
    name: "Rosuvastatin 10mg",
    image: getReliableImage(),
    price: 260,
    description: "Statin medication used to lower cholesterol.",
    expiryDate: "2025-07-25",
    location: "Panjim, Goa",
    category: "Cholesterol",
    isDonation: false
  },
  {
    id: "61",
    name: "Hydrocortisone Cream 1%",
    image: getReliableImage(),
    price: 120,
    description: "Topical corticosteroid used to relieve skin inflammation.",
    expiryDate: "2025-09-09",
    location: "New Delhi, NCR",
    category: "Topical Steroid",
    isDonation: false
  },
  {
    id: "62",
    name: "Budesonide Inhaler",
    image: getReliableImage(),
    price: 380,
    description: "Inhaled corticosteroid used to prevent asthma symptoms.",
    expiryDate: "2025-08-12",
    location: "Bokaro, Jharkhand",
    category: "Asthma",
    isDonation: false
  },
  {
    id: "63",
    name: "Potassium Citrate",
    image: getReliableImage(),
    price: null,
    description: "Supplement used to prevent kidney stones and treat metabolic acidosis.",
    expiryDate: "2025-07-30",
    location: "Noida, Uttar Pradesh",
    category: "Electrolyte",
    isDonation: true
  },
  {
    id: "64",
    name: "Simvastatin 20mg",
    image: getReliableImage(),
    price: 210,
    description: "Statin medication used to lower cholesterol.",
    expiryDate: "2025-06-19",
    location: "Gurgaon, Haryana",
    category: "Cholesterol",
    isDonation: false
  },
  {
    id: "65",
    name: "Spironolactone 25mg",
    image: getReliableImage(),
    price: 195,
    description: "Diuretic used to treat high blood pressure and fluid retention.",
    expiryDate: "2025-09-14",
    location: "Faridabad, Haryana",
    category: "Diuretic",
    isDonation: false
  },
  {
    id: "66",
    name: "Salbutamol Inhaler",
    image: getReliableImage(),
    price: 350,
    description: "Bronchodilator used to relieve asthma symptoms.",
    expiryDate: "2025-07-15",
    location: "Meerut, Uttar Pradesh",
    category: "Asthma",
    isDonation: false
  },
  {
    id: "67",
    name: "Glucosamine 500mg",
    image: getReliableImage(),
    price: null,
    description: "Supplement used to support joint health.",
    expiryDate: "2025-10-08",
    location: "Bhiwadi, Rajasthan",
    category: "Joint Health",
    isDonation: true
  },
  {
    id: "68",
    name: "Dutasteride 0.5mg",
    image: getReliableImage(),
    price: 285,
    description: "5-alpha reductase inhibitor used to treat enlarged prostate.",
    expiryDate: "2025-08-17",
    location: "Haridwar, Uttarakhand",
    category: "Prostate",
    isDonation: false
  },
  {
    id: "69",
    name: "Allopurinol 100mg",
    image: getReliableImage(),
    price: 165,
    description: "Xanthine oxidase inhibitor used to treat gout.",
    expiryDate: "2025-07-28",
    location: "Rohtak, Haryana",
    category: "Gout",
    isDonation: false
  },
  {
    id: "70",
    name: "Buspirone 10mg",
    image: getReliableImage(),
    price: 230,
    description: "Anxiolytic used to treat anxiety disorders.",
    expiryDate: "2025-06-15",
    location: "Aligarh, Uttar Pradesh",
    category: "Anti-Anxiety",
    isDonation: false
  },
  {
    id: "71",
    name: "Vitamin E 400 IU",
    image: getReliableImage(),
    price: null,
    description: "Antioxidant supplement to support overall health.",
    expiryDate: "2025-09-19",
    location: "Hisar, Haryana",
    category: "Vitamin",
    isDonation: true
  },
  {
    id: "72",
    name: "Celecoxib 200mg",
    image: getReliableImage(),
    price: 275,
    description: "NSAID used to treat pain and inflammation.",
    expiryDate: "2025-08-11",
    location: "Saharanpur, Uttar Pradesh",
    category: "Anti-inflammatory",
    isDonation: false
  },
  {
    id: "73",
    name: "Doxazosin 4mg",
    image: getReliableImage(),
    price: 220,
    description: "Alpha-blocker used to treat high blood pressure and enlarged prostate.",
    expiryDate: "2025-07-12",
    location: "Bareilly, Uttar Pradesh",
    category: "Blood Pressure",
    isDonation: false
  },
  {
    id: "74",
    name: "Budesonide Nasal Spray",
    image: getReliableImage(),
    price: 340,
    description: "Nasal corticosteroid used to treat allergy symptoms.",
    expiryDate: "2025-06-08",
    location: "Roorkee, Uttarakhand",
    category: "Allergy",
    isDonation: false
  },
  {
    id: "75",
    name: "Probiotic Sachets",
    image: getReliableImage(),
    price: null,
    description: "Supplement to support digestive health.",
    expiryDate: "2025-10-15",
    location: "Karnal, Haryana",
    category: "Digestive Health",
    isDonation: true
  },
  {
    id: "76",
    name: "Finasteride 1mg",
    image: getReliableImage(),
    price: 310,
    description: "5-alpha reductase inhibitor used to treat hair loss and enlarged prostate.",
    expiryDate: "2025-09-22",
    location: "Mathura, Uttar Pradesh",
    category: "Hair Loss",
    isDonation: false
  },
  {
    id: "77",
    name: "Prednisolone 5mg",
    image: getReliableImage(),
    price: 185,
    description: "Corticosteroid used to treat inflammation and autoimmune conditions.",
    expiryDate: "2025-08-19",
    location: "Ghaziabad, Uttar Pradesh",
    category: "Steroid",
    isDonation: false
  },
  {
    id: "78",
    name: "Ramipril 5mg",
    image: getReliableImage(),
    price: 205,
    description: "ACE inhibitor used to treat high blood pressure and heart failure.",
    expiryDate: "2025-07-14",
    location: "Rewari, Haryana",
    category: "Blood Pressure",
    isDonation: false
  },
  {
    id: "79",
    name: "Multivitamin Gummies",
    image: getReliableImage(),
    price: null,
    description: "Chewable multivitamin supplement for children and adults.",
    expiryDate: "2025-10-11",
    location: "Moradabad, Uttar Pradesh",
    category: "Vitamin",
    isDonation: true
  },
  {
    id: "80",
    name: "Tamsulosin 0.4mg",
    image: getReliableImage(),
    price: 250,
    description: "Alpha-blocker used to treat enlarged prostate.",
    expiryDate: "2025-09-18",
    location: "Mandi, Himachal Pradesh",
    category: "Prostate",
    isDonation: false
  },
  {
    id: "81",
    name: "Naproxen 500mg",
    image: getReliableImage(),
    price: 160,
    description: "NSAID used to treat pain and inflammation.",
    expiryDate: "2025-08-15",
    location: "Dharamshala, Himachal Pradesh",
    category: "Painkiller",
    isDonation: false
  },
  {
    id: "82",
    name: "Salbutamol Nebules",
    image: getReliableImage(),
    price: 295,
    description: "Bronchodilator used to relieve asthma symptoms via nebulizer.",
    expiryDate: "2025-07-26",
    location: "Kullu, Himachal Pradesh",
    category: "Asthma",
    isDonation: false
  },
  {
    id: "83",
    name: "Calcium + Vitamin D3",
    image: getReliableImage(),
    price: null,
    description: "Supplement to support bone health.",
    expiryDate: "2025-09-27",
    location: "Nainital, Uttarakhand",
    category: "Bone Health",
    isDonation: true
  },
  {
    id: "84",
    name: "Ketoconazole Shampoo",
    image: getReliableImage(),
    price: 225,
    description: "Antifungal shampoo used to treat dandruff and fungal infections.",
    expiryDate: "2025-08-24",
    location: "Mussoorie, Uttarakhand",
    category: "Antifungal",
    isDonation: false
  },
  {
    id: "85",
    name: "Amoxicillin + Clavulanic Acid",
    image: getReliableImage(),
    price: 340,
    description: "Antibiotic combination used to treat bacterial infections.",
    expiryDate: "2025-07-21",
    location: "Pithoragarh, Uttarakhand",
    category: "Antibiotic",
    isDonation: false
  },
  {
    id: "86",
    name: "Rifaximin 550mg",
    image: getReliableImage(),
    price: 390,
    description: "Antibiotic used to treat traveler's diarrhea and hepatic encephalopathy.",
    expiryDate: "2025-06-18",
    location: "Haldwani, Uttarakhand",
    category: "Antibiotic",
    isDonation: false
  },
  {
    id: "87",
    name: "Echinacea Extract",
    image: getReliableImage(),
    price: null,
    description: "Herbal supplement to support immune function.",
    expiryDate: "2025-10-14",
    location: "Solan, Himachal Pradesh",
    category: "Immunity",
    isDonation: true
  },
  {
    id: "88",
    name: "Levocetirizine 5mg",
    image: getReliableImage(),
    price: 125,
    description: "Antihistamine used to relieve allergy symptoms.",
    expiryDate: "2025-09-15",
    location: "Bilaspur, Himachal Pradesh",
    category: "Antihistamine",
    isDonation: false
  },
  {
    id: "89",
    name: "Calcitriol 0.25mcg",
    image: getReliableImage(),
    price: 210,
    description: "Active form of vitamin D used to treat hypocalcemia.",
    expiryDate: "2025-08-12",
    location: "Hamirpur, Himachal Pradesh",
    category: "Vitamin D",
    isDonation: false
  },
  {
    id: "90",
    name: "Clobetasol Propionate 0.05%",
    image: getReliableImage(),
    price: 175,
    description: "Topical corticosteroid used to relieve skin inflammation.",
    expiryDate: "2025-07-09",
    location: "Una, Himachal Pradesh",
    category: "Topical Steroid",
    isDonation: false
  },
  {
    id: "91",
    name: "Garlic Extract",
    image: getReliableImage(),
    price: null,
    description: "Herbal supplement to support cardiovascular health.",
    expiryDate: "2025-10-06",
    location: "Chamba, Himachal Pradesh",
    category: "Supplement",
    isDonation: true
  },
  {
    id: "92",
    name: "Minoxidil Solution 5%",
    image: getReliableImage(),
    price: 320,
    description: "Topical medication used to treat hair loss.",
    expiryDate: "2025-09-03",
    location: "Tehri, Uttarakhand",
    category: "Hair Growth",
    isDonation: false
  },
  {
    id: "93",
    name: "Olmesartan 20mg",
    image: getReliableImage(),
    price: 215,
    description: "ARB used to treat high blood pressure.",
    expiryDate: "2025-08-30",
    location: "Rudraprayag, Uttarakhand",
    category: "Blood Pressure",
    isDonation: false
  },
  {
    id: "94",
    name: "Terbinafine Cream",
    image: getReliableImage(),
    price: 165,
    description: "Antifungal cream used to treat skin infections.",
    expiryDate: "2025-07-16",
    location: "Chamoli, Uttarakhand",
    category: "Antifungal",
    isDonation: false
  },
  {
    id: "95",
    name: "Ginger Extract",
    image: getReliableImage(),
    price: null,
    description: "Herbal supplement to support digestive health and reduce nausea.",
    expiryDate: "2025-10-22",
    location: "Kinnaur, Himachal Pradesh",
    category: "Supplement",
    isDonation: true
  },
  {
    id: "96",
    name: "Lisinopril 10mg",
    image: getReliableImage(),
    price: 190,
    description: "ACE inhibitor used to treat high blood pressure and heart failure.",
    expiryDate: "2025-09-12",
    location: "Lahaul and Spiti, Himachal Pradesh",
    category: "Blood Pressure",
    isDonation: false
  }
];
